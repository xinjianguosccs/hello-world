...
<body>
	<table> 
 	 	...
 	 	<tr><?php include("classesmenu.php");?>
 	 		...
 	 		<form method="post" action=assignextrastudents.php>  		
<!--start content  -->
...
if( $btassign=='Assign Students'){
	...
	$ClassID = $hdclassid;
	header("location:assignextrastudents.php?ClassID=".$ClassID );
	exit();	
}
...
?>
			<table border="1" cellspacing="0">
				<tr><td bgcolor='powderblue' colspan='6' class='textlargeBoldBrown'>Class Info<input type="hidden" name="hdclassid" value="<?php echo $ClassID;?>"/></td></tr>
				<tr>
...
<!-- display assigned students  -->		    		
			<table border="1" cellspacing="0">
				<tr><td bgcolor='powderblue' colspan='4' class='textlargeBoldBrown'>Assigned Students</td></tr>
				<tr>
...
<!--display unassigned students -->
			<table border="1" cellspacing="0">
				<tr><td bgcolor='powderblue' colspan='5' class='textlargeBoldBrown'>Unassigned Students</td></tr>
				...
				<tr class='textsmallheadBrown'>
					...
					<td align='center' bgcolor='darkgray'>Preferred<br/>Extra Class</td>
				</tr>
...
$query = "select a.MemberID,
                 ...
                 a.PreferredExtraClass
          from (tblStudent A join tblMember B on a.memberID=b.memberID) 
          WHERE b.registered='yes'
            and a.PreferredExtraClass='" .$classRow[GradeOrSubject]. "'  
            and a.MemberID not in  (SELECT StudentMemberID FROM tblClassRegistration join tblClass on tblClass.ClassID=tblClassRegistration.ClassID WHERE (not tblClass.Gradeorsubject REGEXP '[[:digit:]]+') and  tblClass.Year='".$CurrentYear."')
           ORDER BY b.LastName";     
...
?>
				<tr class='textsmallblack'>
					...
					<td><?php echo $row[PreferredExtraClass];?>&nbsp;</td>
				</tr>
<?php
}
mysqli_close($conn);
...
?>
				<tr class='textsmallblack'>
					<td colspan='5'>No student unassigned</td>
				</tr>
				<tr class='textsmallblack'>
					<td colspan='5'><input type="submit" name="assign" value="Assign Students" disabled /></td>
				</tr>				
<?php
}else{
?>
				<tr class='textsmallblack'>
					<td colspan='5'><input type="submit" name="assign" value="Assign Students"/></td>
				</tr>
<?php
}
?>
